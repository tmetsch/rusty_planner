...
pub fn solve<PS: Problem>(ps: &mut PS, steps: i32) -> (bool, i32) {
    ...
    for i in 0..steps {
        ...
        for item in ps.find_conflicts() {
            heap.push(util::HeapEntry { state: item.0, keys: (item.1, 0.0) });
        }
        if !heap.is_empty() {
            ...
            for (iden, chan) in self.channels.iter() {
                ...
                if rem > 0 {
                    for i in 0..(SIZE - rem) {
                        if chan.iden == self.channels[&(iden + i + 1)].iden {
                            res.push(((*iden, (iden + i + 1)),
                                      -1.0 * (chan.iden -
                                          self.channels[&(iden + i + 1)].iden) as f64));
                        }
                    }
                }
                // ngbh below me...
                if (iden + SIZE) < self.channels.len() as i32 {
                    if chan.iden == self.channels[&(iden + SIZE)].iden {
                        res.push(((*iden, (iden + SIZE)),
                                  -1.0 * (chan.iden -
                                      self.channels[&(iden + SIZE)].iden) as f64));
                    }
                }
            }
            res.into_iter()
